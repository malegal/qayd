...
window.addProfessionalDocument = async function(id) { const file = await db.officeFiles.get(id); if (!file || !ipcRenderer?.selectProfessionalDocument) return; const source = await ipcRenderer.selectProfessionalDocument(); if (!source) return; const result = await ipcRenderer.copyProfessionalDocument(source, file.file_code, file.client_name, file.file_type); if (!result?.success) return Swal.fire('خطأ', result?.error || 'تعذر إضافة المستند', 'error'); Swal.fire({ icon: 'success', title: 'تمت إضافة المستند', text: result.name, timer: 1500, showConfirmButton: false }); };
...
// اختيار مستند من الجهاز ونسخه إلى مجلد مستندات القضية مع الحفاظ على الملف الأصلي.
window.addCaseDocument = async function() {
    ...
    const c = await db.cases.get(activeCaseId);
    if (!c) return;
    const sourcePath = await ipcRenderer.selectCaseDocument();
    if (!sourcePath) return;
    const result = await ipcRenderer.copyCaseDocument(sourcePath, c.case_code, c.client_name);
    if (!result?.success) return Swal.fire('خطأ', result?.error || 'تعذر نسخ المستند', 'error');
