...
window.editProfessionalFile = async function(id) { const file = await db.officeFiles.get(id); if (!file) return; const f = file.metadata?.followup || {}; document.getElementById('editingProfessionalFileId').value = id; document.getElementById('professionalFileType').value = file.file_type; document.getElementById('professionalFileTitle').value = file.title || ''; document.getElementById('professionalClientName').value = file.client_name || ''; document.getElementById('professionalClientPhone').value = file.client_phone || ''; document.getElementById('professionalFileDescription').value = file.description || ''; document.getElementById('professionalLastActionDate').value = f.last_action_date || ''; document.getElementById('professionalLastAction').value = f.last_action || ''; document.getElementById('professionalNextActionDate').value = f.next_action_date || ''; document.getElementById('professionalNextAction').value = f.next_action || ''; document.getElementById('professionalFileSaveButton').textContent = 'حفظ التعديلات'; showModal('professionalFileModal'); };
...
window.selectProfessionalFile = async function(id) {
    ...
    activeCaseId = id;
    const followup = file.metadata?.followup || {}; document.getElementById('caseDetailContent').innerHTML = `<div class="mb-2"><span class="text-warning fw-bold">كود الملف:</span> <span class="badge bg-dark text-warning border border-warning px-2 py-1">${escapeHtml(file.file_code)}</span></div><div><strong>العميل:</strong> ${escapeHtml(file.client_name)}</div><div><strong>النوع:</strong> ${professionalTypeLabel(file.file_type)}</div><div><strong>اسم الملف:</strong> ${escapeHtml(file.title)}</div><div><strong>الحالة:</strong> ${escapeHtml(file.status)}</div><div><strong>الوصف:</strong> ${escapeHtml(file.description || '-')}</div><hr><div><strong>آخر إجراء:</strong> ${escapeHtml(followup.last_action || '-')} ${followup.last_action_date ? `(${escapeHtml(followup.last_action_date)})` : ''}</div><div><strong>الإجراء القادم:</strong> ${escapeHtml(followup.next_action || '-')} ${followup.next_action_date ? `(${escapeHtml(followup.next_action_date)})` : ''}</div>`;
    ...
    document.getElementById('caseActionsPanel').innerHTML = `<span class="badge bg-success">ملف إجرائي/خدمي — له إجراءات ومواعيد متابعة</span><button class="btn btn-sm btn-outline-primary" onclick="editProfessionalFile('${file.id}')"><i class="bi bi-pencil"></i> تعديل</button><button class="btn btn-sm btn-outline-success" onclick="openProfessionalActionModal('${file.id}')"><i class="bi bi-calendar-plus"></i> إجراء</button><button class="btn btn-sm btn-outline-info" onclick="addProfessionalDocument('${file.id}')"><i class="bi bi-file-earmark-plus"></i> مستند</button><button class="btn btn-sm btn-outline-secondary" onclick="openProfessionalFolder('${file.id}')"><i class="bi bi-folder-fill"></i> مجلد</button><button class="btn btn-sm btn-outline-success" onclick="openFeesModal('${file.id}')"><i class="bi bi-cash-stack"></i> الأتعاب والمصروفات</button>`;
...
window.openCaseDetails = async function(id) { if (!id) return; try { const c = await db.cases.get(id); if (!c) return; activeCaseId = id; document.getElementById('caseDetailContent').innerHTML = `<div class="d-flex justify-content-between align-items-center mb-3"><div><span class="client-name-large">${c.client_name}</span><span class="role-badge me-3">${c.client_role || 'صفة غير محددة'}</span></div><span class="fs-5 text-warning fw-bold border border-warning px-3 py-1 rounded bg-dark">${c.case_code || '-'}</span></div><div class="row mt-3 text-white fs-5"><div class="col-6 mb-3"><strong>رقم القضية:</strong> <span class="text-light">${c.case_number} / ${c.case_year}</span></div><div class="col-6 mb-3"><strong>المحكمة/الدائرة:</strong> <span class="text-light">${c.court_name || ''} ${c.circuit ? '- الدائرة ' + c.circuit : ''}</span></div><div class="col-6 mb-3"><strong>نوع القضية:</strong> <span class="text-light">${c.case_type || 'غير محدد'}</span></div><div class="col-6 mb-3"><strong>الخصم:</strong> <span class="text-light">${c.opponent_name || '-'}</span></div><div class="col-6 mb-3"><strong>الهاتف:</strong> <span class="text-light">${c.client_phone || '-'}</span></div><div class="col-6 mb-3"><strong>عنوان العميل:</strong> <span class="text-light">${c.client_address || '-'}</span></div><div class="col-6 mb-3"><strong>هاتف الخصم:</strong> <span class="text-light">${c.opponent_phone || '-'}</span></div><div class="col-6 mb-3"><strong>عنوان الخصم:</strong> <span class="text-light">${c.opponent_address || '-'}</span></div><div class="col-12 mt-2"><strong>الموضوع:</strong> <br><span class="text-light">${c.case_subject || '-'}</span></div></div>`; const sessions = await db.sessions.where('case_id').equals(id).toArray(); sessions.sort((a, b) => new Date(b.session_date) - new Date(a.session_date)); currentCaseForPrint = { ...c, sessions: sessions }; let sHtml = ''; if (sessions.length > 0) { for (let s of sessions) { sHtml += `<div class="session-item-row"><div><span class="text-info fw-bold fs-5">${new Date(s.session_date).toLocaleString('ar-EG', { dateStyle: 'full', timeStyle: 'short' })}</span><span class="badge bg-light text-dark mx-3 fs-6">${s.case_status}</span><div class="fs-6 mt-2 text-white">${s.decision || 'لا يوجد قرار مسجل'}</div></div><button class="btn btn-sm btn-outline-warning" onclick="event.stopPropagation(); openEditSession('${s.id}')"><i class="bi bi-pencil fs-5"></i></button></div>`; } } else { sHtml = '<p class="text-muted">لا توجد جلسات مسجلة لهذه القضية.</p>'; } document.getElementById('caseDetailSessions').innerHTML = sHtml; document.getElementById('caseActionsPanel').innerHTML = `${ipcRenderer && ipcRenderer.createCaseFolder ? '<button class="btn btn-outline-secondary" onclick="handleOpenCaseFolder()"><i class="bi bi-folder-fill"></i> مجلد</button>' : ''}<button class="btn btn-outline-primary" onclick="showCasePanelSection('sessions')"><i class="bi bi-calendar-check"></i> الجلسات</button><button class="btn btn-outline-primary" onclick="showCasePanelSection('data')"><i class="bi bi-card-text"></i> البيانات</button><button class="btn btn-outline-info" onclick="openClientDetails()"><i class="bi bi-person-vcard"></i> بيانات العميل</button><button class="btn btn-success" onclick="openFeesModal(activeCaseId)"><i class="bi bi-cash-coin"></i> الأتعاب</button><button class="btn btn-outline-info" onclick="addCaseDocument()"><i class="bi bi-file-earmark-plus"></i> مستندات</button><button class="btn btn-outline-warning" onclick="openEditCaseModalFromPanel()"><i class="bi bi-pencil"></i> تعديل</button><button class="btn btn-outline-danger" onclick="showCaseOptions()"><i class="bi bi-archive"></i> أرشفة</button>`; showCasePanelSection('data'); } catch (e) { console.error(e); } };
...
window.openEditCaseModalFromPanel = async function() {
    const c = await db.cases.get(activeCaseId); if (!c) return;
    const values = { edit_c_name: c.client_name, edit_c_phone: c.client_phone, edit_c_address: c.client_address, edit_c_opponent: c.opponent_name, edit_c_opponent_phone: c.opponent_phone, edit_c_opponent_address: c.opponent_address, edit_c_num: c.case_number, edit_c_year: c.case_year, edit_c_court: c.court_name, edit_c_circuit: c.circuit, edit_case_type: c.case_type, edit_c_subject: c.case_subject };
...
window.openProfessionalFileModal = async function() {
    ...
    ['professionalFeeTotal','professionalFeePaid','professionalFeeNotes','professionalExpenseAmount','professionalExpenseCategory','professionalLastActionDate','professionalLastAction','professionalNextActionDate','professionalNextAction'].forEach(id => { const el = document.getElementById(id); if (el) el.value = ''; });
...
window.saveProfessionalFile = async function() {
    try {
        ...
        const file = { id: editingId || generateUUID(), office_id: currentOfficeId, file_code: editingId ? (await db.officeFiles.get(editingId)).file_code : fileCode, file_type: fileType, title, client_name: clientName, client_phone: clientPhone, status: 'مفتوح', description: description || null, metadata: editingId ? ((await db.officeFiles.get(editingId)).metadata || {}) : {}, archived: false, created_at: editingId ? (await db.officeFiles.get(editingId)).created_at : new Date().toISOString(), updated_at: new Date().toISOString() };
